  ...
  async function handleRestore() {
    try {
      ...
      Alert.alert("Restore backup?", "This will replace all your current local data.", [
        ...
        { text: "Restore", style: "destructive", onPress: () => { commit(payload.data); Alert.alert("Restored", "Data restored successfully."); } },
      ]);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <TouchableOpacity onPress={() => navigate(ROUTES.DASHBOARD)} style={styles.backButton}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.listItemStacked}>
            ...
            <Text style={styles.itemSub}>You will be notified {profile.alertDays || 30} days before a document expires.</Text>
          </View>
        ...
        <View style={styles.footer}><Text style={styles.footerText}>DocuTrak v1.0.0</Text></View>
      </ScrollView>
      ...
      <Modal visible={editModal.visible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <TextInput style={styles.modalInput} value={editModal.value} onChangeText={(t) => setEditModal(p => ({ ...p, value: t }))} placeholder={`Enter ${editModal.field.toLowerCase()}...`} placeholderTextColor={colors.textMuted} autoFocus />
...
const styles = StyleSheet.create({
  ...
  backButton: { marginRight: 16, padding: 8, borderRadius: 20, backgroundColor: colors.surfaceElevated },
  ...
  listItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.surfaceElevated, padding: 18, borderRadius: 20, marginBottom: 8, borderWidth: 1, borderColor: colors.border },
  listItemStacked: { backgroundColor: colors.surfaceElevated, padding: 18, borderRadius: 20, marginBottom: 8, borderWidth: 1, borderColor: colors.border },
  ...
  alertDaysBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.primary, justifyContent: 'center', alignItems: 'center' },
  alertDaysValue: { fontSize: 20, fontWeight: '900', color: colors.text, minWidth: 40, textAlign: 'center' },
  ...
  modalCard: { width: '100%', backgroundColor: colors.surfaceElevated, borderRadius: 24, padding: 24, borderWidth: 1, borderColor: colors.border },
  modalTitle: { fontSize: 20, fontWeight: '800', color: colors.text, marginBottom: 16 },
  modalInput: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14, fontSize: 16, color: colors.text, marginBottom: 20 },
  modalActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 12 },
  modalCancelBtn: { paddingVertical: 12, paddingHorizontal: 20, borderRadius: 12, backgroundColor: colors.surface },
  modalCancelText: { color: colors.textMuted, fontWeight: '700', fontSize: 15 },
  modalSaveBtn: { paddingVertical: 12, paddingHorizontal: 24, borderRadius: 12, backgroundColor: colors.primary },
